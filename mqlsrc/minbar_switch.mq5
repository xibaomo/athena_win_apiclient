...
int OnInit()
{
    ...
    PrintFormat("time at mt5: %d, at server %d",latestRate[0].time,histTime);
...
int sendPastMinBars(datetime histTime,int histLen)
{
    ...
    for (k=0; k<histLen; k++) {
        if (rates[k].time == histTime) {
            PrintFormat("Found latest server bar at %d",k);
            PrintFormat("Next bar time: %s",TimeToString(rates[k+1].time));
            break;
        }
    }
    int actualHistLen = histLen - k-1;
    int idx=k;
    float data[];
